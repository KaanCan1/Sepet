 ...
 *
 * İLK İKİ TESTE AÇIK ZAMAN AŞIMI VERİLDİ. Her biri elli yedi etiketli vaka
 * için ayrı ayrı veritabanına gidiyor; bulanık eşleştirme satır başına
 * milisaniyeler sürüyor ve toplam saniyelerle ölçülüyor. Vitest'in beş
 * saniyelik varsayılanı bu iş için yanlış ölçek: CI'da düşen sürüm zaten
 * 5006 ms'de kalmıştı, yani test aylardır kılpayı geçiyordu ve düşmesi için
 * puanlamaya yüzde on eklenmesi yetti.
 *
 * Süre bir performans bekleyişi DEĞİL — bu testlerin ölçtüğü şey kalite.
 * Hız için ayrı bir ölçüm gerekiyorsa yeri burası değil.
 */
describe("Eşleştirme kalitesi", () => {
  it(
    "etiketli kümede hiç yanlış bağlama yok",
    { timeout: 30_000 },
    async () => {
      const yanlis: string[] = [];
  ...
  it(
    "otomatik eşleşme oranı tabanın altına düşmüyor",
    { timeout: 30_000 },
    async () => {
      let dogru = 0;
      for (const v of MATCH_CASES) {
  ...
  it(
    "katalogda olmayan ürün otomatik bağlanmıyor",
    { timeout: 30_000 },
    async () => {
